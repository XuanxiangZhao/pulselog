...
function bindEvents() {
  ...
  els.authSignIn.addEventListener("click", signInWithEmail);
...
async function signInWithEmail() {
  if (!supabaseClient) {
    alert("请先在 supabase-config.js 里填写 Supabase URL 和 anon key。");
    return;
  }
...
async function handleCloudSession(session) {
  ...
  els.authSignOut.hidden = !currentUser;
  els.authSignIn.hidden = Boolean(currentUser);
  ...
  setSyncStatus("同步中");
  els.cloudUser.textContent = currentUser.email || "已登录";
...
async function loadCloudState() {
  ...
  state = Object.assign(cloneDefaultState(), data.data);
...
function subscribeCloudState() {
  ...
  realtimeChannel = supabaseClient
    .channel(`pulselog-state-${currentUser.id}`)
    .on(
      "postgres_changes",
      {
        event: "*",
        schema: "public",
        table: "pulselog_state",
        filter: `user_id=eq.${currentUser.id}`,
      },
      (payload) => {
        if (!payload.new || !payload.new.data) return;
        isApplyingRemoteState = true;
        state = Object.assign(cloneDefaultState(), payload.new.data);
...
function queueCloudSave() {
  if (!supabaseClient || !currentUser || isApplyingRemoteState) return;
  setSyncStatus("保存中");
...
async function saveCloudStateNow() {
  ...
  const { error } = await supabaseClient.from("pulselog_state").upsert({
